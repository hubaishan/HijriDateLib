...
$hijri_settings=array(
   ...
   'type' => 'file',  // can be 'mysql' or 'file'
     'file_path'=> 'adj.txt' //file path
      ));
...
$adj=new hijri\calendar();

//     function get_adjdata() {
//        global $adj_data_source;
//         $adj = array();
//        if ($adj_data_source['type'] == "file") {
//            if (file_exists($adj_data_source['file_path'])) {
//                eval('$adj=array(' . file_get_contents($adj_data_source['file_path']) . ');');
//            }
//        } elseif ($adj_data_source['type'] == "mysql") {
//            eval('$cnxn= new mysqli(' . $adj_data_source['connection'] . ");");
//            $result = $cnxn->query($adj_data_source['get_sql']);
//            $row = $result->fetch_row();
//            eval('$adj=array(' . $row[0] . ');');
//
//
//            $cnxn->close();
//        }
...
//
//        if ($adj_data_source['type'] == "file") {
//            $myret = file_exists(file_put_contents($adj_data_source['file_path'], $mytext));
//        } elseif ($adj_data_source['type'] == "mysql") {
//            eval('$cnxn= new mysqli(' . $adj_data_source['connection'] . ");");
//            $myret = $cnxn->query(sprintf($adj_data_source['set_sql'], $mytext));
//            $cnxn->close();
//        }
//        return $myret;
//    }

    function off2month($off) {
    $ii = floor(($off) / 12);
    $hy = umstartyear + $ii;
    $hm = $off + 1 - 12 * $ii;
    return "$hm/$hy";
...
function month2off($month, $year) {
    $ii = $year - umstartyear;
    $i = $month - 1 + 12 * $ii;
    return $i;
}

function myjd2gre($jd) {
    list($gm, $gd, $gy) = explode("/", jdtogregorian(2400000 + $jd));
    return "$gd/$gm/$gy";
}

function check_auto_del($off) {
    global $um_data;
    $my_adj = $_SESSION['adj_data'];
    unset($my_adj[$off]);
    $um_data_adj = array_replace($um_data, $my_adj);
    $myret = array();
    for ($noff = $off + 1; array_key_exists($noff, $my_adj); $noff++) {
        if ((($um_data_adj[$noff] - $um_data_adj[$noff - 1]) < 29) or ( ($um_data_adj[$noff] - $um_data_adj[$noff - 1]) > 30)) {
            $myret[] = $noff;
            unset($my_adj[$noff]);
            $um_data_adj = array_replace($um_data, $my_adj);
        } else {
...
function check_auto_adj($off, $v) {
    ...
    //print_r($um_data_adj);
    $myret = array();
    for ($noff = $off + 1;array_key_exists($noff, $um_data); $noff++) {
        if (($um_data_adj[$noff] - $um_data_adj[$noff - 1]) < 29) {

            $myret[$noff] = $um_data_adj[$noff - 1] + 29;
            $um_data_adj = array_replace($um_data, $my_adj, $myret);
...
if (!empty($_POST['login'])) {
    ...
    header("Location: " . $_SERVER["SCRIPT_NAME"] . "?action=add&idx=" . month2off($_GET[month], $_GET[year]));
    exit;
} elseif (array_key_exists('savechanges', $_POST)) {
    $myret=$adj->set_adjdata($_SESSION['adj_data']);
	header("Location: " . $_SERVER["SCRIPT_NAME"]);
    exit;
    ...
    $v = $_POST['v'];
    if ($um_data[$off] == $v) {
        unset($_SESSION['adj_data'][$off]);
    } else {
        $_SESSION['adj_data'][$off] = $v;
    }
    $_SESSION['adj_data'] = array_replace($_SESSION['adj_data'], check_auto_adj($off, $v));
    asort($_SESSION['adj_data'], SORT_NUMERIC);
    header("Location: " . $_SERVER["SCRIPT_NAME"]);
    exit;
} elseif (array_key_exists('deladj', $_POST)) {
    $off = $_POST['off'];
    unset($_SESSION['adj_data'][$off]);
    $auto_del = check_auto_del($off);
    foreach ($auto_del as $k) {
        unset($_SESSION['adj_data'][$k]);
    }
    //asort($_SESSION['adj_data'],SORT_NUMERIC);
    header("Location: " . $_SERVER["SCRIPT_NAME"]);
    exit;
...
if (array_key_exists('adj_data', $_SESSION)) {
    $adj_data = $_SESSION['adj_data'];
} else {
    $adj_data = $adj->get_adjdata();
    $_SESSION['adj_data'] = $adj_data;
}
$um_data_adj = array_replace($um_data, $adj_data);

if (!empty($_GET['action'])) {
    if ($_GET['action'] == 'del') {
        echo "هل تريد بالتأكيد حذف تعديل " . off2month($_GET['idx']);
        $auto_del = check_auto_del($_GET['idx']);
        if (count($auto_del) > 0) {
        ...
        echo '<form method="post"><input type="hidden" name="deladj" value=1><input type="hidden" name="off" value=' . $_GET['idx'] . '><input type="submit" name="submit" value="نعم بالتأكيد" /></from>';
        echo '<a href="' . $_SERVER['SCRIPT_NAME'] . '">إلغاء</a>';
        //unset($_SESSION['adj_data'][$_GET['idx']]
    } elseif ($_GET['action'] == 'edit' or $_GET['action'] == 'add') {
        $off = $_GET['idx'];
        list($hm, $hy) = explode("/", off2month($off));
        echo "تعديل بداية الشهر " . $hmonths[$hm - 1] . " من سنة $hy إلى:";
        echo '<form method="post"><input type="hidden" name="addadj" value=1><input type="hidden" name="off" value=' . $off . '><select name="v">';
        for ($n = $um_data_adj[$off - 1] + 29; $n < $um_data_adj[$off - 1] + 31; $n++) {

            echo "<option value=$n";
            if ($n == $um_data_adj[$off]) {
                echo " selected ";
            }
            echo ">" . myjd2gre($n);
            $auto_adj = check_auto_adj($off, $n);
            foreach ($auto_adj as $k => $v) {
                list($hm, $hy) = explode("/", off2month($k));
                echo " وسيتم أيضا تعديل بداية شهر " . $hmonths[$hm - 1] . " من سنة $hy إلى:" . myjd2gre($v);
            }
            echo "</option>";
        }
    ...
    echo $msg . '<br/>';
    foreach ($adj_data as $k => $v) {
        echo off2month($k) . " => " . myjd2gre($v) . " الافتراضي هو " . myjd2gre($um_data[$k]) . " [<a href='" . $_SERVER['SCRIPT_NAME'] . "?action=del&amp;idx=$k'>حذف</a>] [<a href='" . $_SERVER['SCRIPT_NAME'] . "?action=edit&amp;idx=$k'>تعديل</a>]<br/>";
    }
    ...
    for ($n = 1; $n < 13; $n++) {
        ...
        echo ">" . $hmonths[$n - 1] . "</option>\n";
    }
    echo '</select><input type="submit" name="add" value="طلب إضافة" /></form>';
    echo '<br/>بيانات ملف التعديل<br/><textarea rows="6" cols="50" style="text-align:left;direction: ltr;">';
    foreach ($adj_data as $k => $v) {
        echo "$k => $v,\n";
    }
    echo '</textarea><br/><form method="post"><input type="submit" name="savechanges" value="حفظ التعديلات" /></form>';
