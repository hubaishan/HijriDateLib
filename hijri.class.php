...
class datetime extends \DateTime {
	 ...
	 * @param string		$langcode	set the language which can be any of supported languages in Calendar object if not set the default is 'ar'
	 ...
	 * @param \DateTimeZone $timezone	Optional the time zone object
	 * @param string		$langcode	Optional the langcode
	 * @param Calendar		$hijriCalendar Optional the Calendar object
	...
	public static Function createFromHijri($year, $month, $day, \DateTimeZone $timezone = null, $langcode = null, $hijriCalendar = null) {
		...
		if (isset($hijriCalendar) && (gettype($hijriCalendar) == 'object') && ((get_class($hijriCalendar) == 'hijri\Calendar') || (get_class($hijriCalendar) == 'hijri\CalendarAdjustment'))) {
			$this->hcal = $hijriCalendar;
	 ...
	 * @param	bool	$force_hijri	force the returned date to be Hijri if the $format does not contain underscore (_)
	 * 
...
class Calendar {
	 ...
	 * @ignore 
	 */
	static protected $s_umdata;
	/**
	 * @ignore 
	 */
	static protected $s_adjdata;
	/**
	 *
	 ...
	 * @param  array $settings Optional array contains one or more settings of the hijri 
		...
		if (!empty($settings)) {
			...
			if (isset($settings['adj_data'])) {
				if (is_array($settings['adj_data'])) {
					...
					if (!preg_match('([a-zA-Z\)\(\.\/\[\]])', $settings['adj_data'])) {
						eval('$this->adj_data=array(' . $settings['adj_data'] . ');');
					}
				}
			}
		}
	 ...
	 * @param bool $return_array True to return umdata array, False to load umdata to $umdata variable
	 ...
	 * @param type $langcode the language code 
	 ...
	 * @param	integer $langcode	the language ISO code
	 * @param	bool	$short return short names of months for European languages
...
class CalendarAdjustment Extends Calendar {
	 ...
	 * 
	 * @global array $hijri_settings global hijri calendar settings
	 * @param  array $settings 	Array contains one or more settings of the hijri 
	 ...
	 * @since 2.1.0 
	 * @param	bool	$txt	true to return string like "5324=>21231,3213=>23123" False to return array.
	 * @return	string|array	string adjustment data like "5324=>21231,3213=>23123" if $txt is true, or array of adjustment data if $txt if false
	 */
	 ...
	 * 
	 * @since 2.1.0
	 * @param int $off the index of the month adjustment you want to delete can be given by function month2off
	 * 
	 ...
	 *
	 * @since  2.1.0 
	 * @param	int	$off	The index of the month to be adjusted can be given by function month2off
	 * @param	int	$v		(modified julian day) of the new start of the month
	...
	public function get_possible_starts($month, $year) {
		...
		if ($off > 0 && array_key_exists($off, $this->umdata)) {
			for ($n = $this->umdata[$off - 1] + 29; $n < $this->umdata[$off - 1] + 31; $n++) {
				$auto_adj = array();
	 ...
	 * @since 2.1.0
	 * @param	integer		$month				the Hijri month
	 * @param	integer		$year				the year of Hijri month
	 * @param	int|string	$new_month_start	the new start of the Hijri Month can be integer (modified julian day) or string (Gregorian date (d/m/yyyy) format) 
	...
	public function add_adj($month, $year, $new_month_start) {
		...
		if (is_numeric($new_month_start)) {
			...
			$value = gregoriantojd($gm, $gd, $gy) - 2400000;
		}
